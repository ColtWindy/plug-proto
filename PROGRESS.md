...
현재는 범용 COCO 데이터셋으로 학습된 YOLO 모델을 사용하고 있지만, 재활용품에 특화된 모델이 필요합니다. 실제 현장에서 촬영한 영상 데이터를 수집하고, 페트병, 캔, 비닐, 종이, 플라스틱 등 재활용 가능한 항목별로 라벨링해야 합니다.

데이터 수집은 여러 재활용 선별장에서 다양한 조명, 컨베이어 속도, 물체 배치 상황을 포함해야 정확도를 높일 수 있습니다. Roboflow나 LabelImg 같은 도구로 바운딩 박스 라벨링을 진행하고, 최소 수천 장 이상의 이미지를 확보할 계획입니다. 데이터 증강(rotation, flip, brightness 조정 등)을 통해 학습 데이터를 확장하고, 과적합을 방지합니다.
