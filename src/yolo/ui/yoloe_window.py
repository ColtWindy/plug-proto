...
class YOLOEWindow(QMainWindow):
    """YOLOE 전용 윈도우 (프롬프트 제어 가능)"""
    ...
    def __init__(self, model_manager):
        ...
        self.setWindowTitle("YOLOE - 프롬프트 제어")
    ...
    def _create_model_info(self):
        ...
        self.model_info_label.setStyleSheet("padding: 5px;")
        
        # 초기 모델 정보 표시
        if self.model_manager.current_model:
            model_path = self.model_manager.model_list[0][1] if self.model_manager.model_list else ""
            info_text = self._get_model_info(self.model_manager.current_model, model_path)
            self.model_info_label.setText(info_text)
        
        scroll.setWidget(self.model_info_label)
    ...
    def _get_model_info(self, model, model_path):
        """모델 정보 텍스트 생성"""
        info = []
        info.append(f"<b>파일:</b> {Path(model_path).name}")
        
        # YOLOE 정보
        is_prompt_free = '-pf' in Path(model_path).stem.lower()
        if is_prompt_free:
            info.append(f"<b>타입:</b> YOLOE (prompt-free)")
            info.append(f"<b>프롬프트:</b> 변경 불가")
        else:
            info.append(f"<b>타입:</b> YOLOE")
            info.append(f"<b>프롬프트:</b> 변경 가능")
        
        # Task 정보
        if hasattr(model, 'task'):
            info.append(f"<b>Task:</b> {model.task}")
        ...
        if hasattr(model, 'names'):
            ...
            if isinstance(names, dict):
                ...
                info.append(f"<b>클래스 수:</b> {class_count}")
                
                # 처음 10개만 표시
                sample_classes = list(names.values())[:10]
                info.append(f"<b>클래스:</b> {', '.join(sample_classes)}")
                if class_count > 10:
                    info.append(f"... 외 {class_count - 10}개")
        
        return "<br>".join(info)
    
    def _create_control_buttons(self):
        """제어 버튼 (사용 안 함 - 위젯에서 처리)"""
        pass
    ...
    def _create_source_selector(self):
        ...
        self.video_radio = QRadioButton("비디오 파일")
        self.video_radio.toggled.connect(self._on_source_changed)
    ...
    def _on_model_changed(self, index):
        ...
        # 프롬프트 위젯 업데이트
        self.prompt_widget.update_classes(self.model_manager.current_classes)
